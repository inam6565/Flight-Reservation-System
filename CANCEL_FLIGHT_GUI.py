...
# Top Status bar
status = Label(mainfame, text=" Cancel Flight ", image=f112, compound=LEFT, bg="#ffffff", fg="#0c26af", bd=10,
...
def ticketdelete(cd):
    ...
    with open("new_file.txt", "w") as new_f:
        for line in lines:
            if not line.startswith(str(tkno)):
                new_f.write(line)
    ...
    with open('LIBRARIES\Ticktets_data.txt', 'r') as datafile:
        ...
        for line_no in datafile:
            ...
            if str(tkno) == line_no.split()[0]:
                ...
                messagestr = "Your Refund is Rs. "+str(int(refund))
...
ticketno = Label(line3, text="Enter your Ticket no : ", font=("Arial", 14), image=f128, bg="#0c26af", compound=LEFT,
...
book_button = Button(line3, text="  Cancel Ticket  ", bg="#ffffff", fg="#0c26af", padx=5, pady=5, bd=0,activeforeground="#ffffff",activebackground="#0c26af",
                     command=cancelticketdetails)
